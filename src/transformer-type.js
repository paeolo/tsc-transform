...
const typeResolverTransformer = (program) => {
  ...
  return (context) => {
    ...
    const serializeTypeNode = getTypeSerializer(typeChecker, context);
    ...
    return (sourceFile) => {
      ...
      const serializeTypeOfPropertyDeclaration = (node, currentNameScope) => {
        const objectLiteralExpression = serializeTypeNode(node.type, currentNameScope);

        if (!node.questionToken) {
          const properties = objectLiteralExpression.properties;
          properties.push(
            factory.createPropertyAssignment('required', factory.createTrue())
          );
          factory.updateObjectLiteralExpression(objectLiteralExpression, properties)
        }
      ...
      const serializeTypeOfNode = (node, currentNameScope) => {
        switch (node.kind) {
          case ts.SyntaxKind.PropertyDeclaration:
            return serializeTypeOfPropertyDeclaration(node, currentNameScope);
          default:
